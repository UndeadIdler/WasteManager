...
{
    public partial class AnalysisItem : PageBase, ISingleGridPage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                (Master.FindControl("Panel1") as FineUI.Panel).Title = "分析项目信息";
                BindGrid();
                CheckUserRole();
            }
        }
        ...
        private DataTable GetPagedDataTable()
        {
            ...
            DataTable table2 = DAL.AnalysisItem.QueryAnalysisItem(ser_vDirectiveNumber.Text.Trim(), ser_vSaveNumber.Text.Trim());
        ...
        public void DeleteSelectedRows()
        {
            ...
            int BSuccess = DAL.AnalysisItem.DeleteAnalysisItem(int.Parse(HttpUtility.UrlEncode(keys[0].ToString())));
            if (BSuccess == 1)
            {
        ...
        {
            //if (!beWrite) return "";
            return "Basic/AnalysisItem_Window.aspx";
        }
        ...
        public string GetEditUrl()
        {
            //if (!beWrite) return "";
            object[] keys = Grid1.DataKeys[Grid1.SelectedRowIndex];
            return String.Format("Basic/AnalysisItem_Window.aspx?id={0}", HttpUtility.UrlEncode(keys[0].ToString()));
        }
        ...
        protected void btn_Export_Click(object sender, EventArgs e)
        {
            try
            {
                string filename = "分析项目.xls";
                DataTable table2 = DAL.AnalysisItem.QueryAnalysisItemEx(ser_vDirectiveNumber.Text.Trim(), ser_vSaveNumber.Text.Trim());
                DAL.NPOIHelper.ExportByWebEx(table2, "分析项目表", filename);
