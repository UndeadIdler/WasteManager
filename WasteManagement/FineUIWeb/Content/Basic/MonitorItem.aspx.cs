...
{
    public partial class MonitorItem : PageBase, ISingleGridPage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                (Master.FindControl("Panel1") as FineUI.Panel).Title = "监测项目信息";
                BindGrid();
                CheckUserRole();
            }
        }
        ...
        private DataTable GetPagedDataTable()
        {
            ...
            DataTable table2 = DAL.MonitorItem.QueryMonitorItem(ser_vDirectiveNumber.Text.Trim(), ser_vSaveNumber.Text.Trim());
        ...
        public void DeleteSelectedRows()
        {
            ...
            int BSuccess = DAL.MonitorItem.DeleteMonitorItem(int.Parse(HttpUtility.UrlEncode(keys[0].ToString())));
            if (BSuccess == 1)
            {
        ...
        {
            //if (!beWrite) return "";
            return "Basic/MonitorItem_Window.aspx";
        }
        ...
        public string GetEditUrl()
        {
            //if (!beWrite) return "";
            object[] keys = Grid1.DataKeys[Grid1.SelectedRowIndex];
            return String.Format("Basic/MonitorItem_Window.aspx?id={0}", HttpUtility.UrlEncode(keys[0].ToString()));
        }
        ...
        protected void btn_Export_Click(object sender, EventArgs e)
        {
            try
            {
                string filename = "监测项目.xls";
                DataTable table2 = DAL.MonitorItem.QueryMonitorItemEx(ser_vDirectiveNumber.Text.Trim(), ser_vSaveNumber.Text.Trim());
                DAL.NPOIHelper.ExportByWebEx(table2, "监测项目表", filename);
