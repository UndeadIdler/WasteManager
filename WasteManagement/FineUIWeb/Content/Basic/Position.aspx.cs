...
{
    public partial class Position : PageBase, ISingleGridPage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                (Master.FindControl("Panel1") as FineUI.Panel).Title = "监测位置信息";
                BindGrid();
                CheckUserRole();
            }
        }
        ...
        private DataTable GetPagedDataTable()
        {
            ...
            DataTable table2 = DAL.Position.GetAllPosition();
        ...
        public void DeleteSelectedRows()
        {
            ...
            int BSuccess = DAL.Position.DeletePosition(int.Parse(HttpUtility.UrlEncode(keys[0].ToString())));
            if (BSuccess == 1)
            {
        ...
        {
            //if (!beWrite) return "";
            return "Basic/Position_Window.aspx";
        }
        ...
        public string GetEditUrl()
        {
            //if (!beWrite) return "";
            object[] keys = Grid1.DataKeys[Grid1.SelectedRowIndex];
            return String.Format("Basic/Position_Window.aspx?id={0}", HttpUtility.UrlEncode(keys[0].ToString()));
        }
